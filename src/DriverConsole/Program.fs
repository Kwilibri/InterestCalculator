...
let loanTransactions =
    [| { TransactionDate = DateOnly(2021, 3, 31)
       ...
       { TransactionDate = DateOnly(2021, 4, 23)
         Amount = 3_000_000M
         TransactionType = MoneyLent }
       { TransactionDate = DateOnly(2021, 4, 12)
         Amount = 10_000_000M
         TransactionType = MoneyLent }
       { TransactionDate = DateOnly(2021, 12, 2)
         Amount = 2_000_000M
         TransactionType = MoneyLent }
...
let rateChanges =
    [| { RateStartDate = DateOnly(2022, 1, 28)
         RateInterval = Annually
         Rate = 0.1250M }
       { RateStartDate = DateOnly(2021, 3, 31)
         RateInterval = Annually
         Rate = 0.1200M }
       { RateStartDate = DateOnly(2021, 11, 19)
         RateInterval = Annually
         Rate = 0.1225M }
       { RateStartDate = DateOnly(2022, 3, 25)
         RateInterval = Annually
         Rate = 0.1275M }
       { RateStartDate = DateOnly(2022, 5, 20)
         RateInterval = Annually
         Rate = 0.1325M }
       { RateStartDate = DateOnly(2022, 7, 21)
         RateInterval = Annually
         Rate = 0.1400M }
...
let calculateInterestChangeLoanAtoms (loan: Loan, calculationEndDate: DateOnly) =
    let sortedRateChanges =
        loan.RateChanges
        |> Array.sortBy (fun x -> x.RateStartDate)
    ...
    let result =
        if (firstKnownRateDate > firstTransactionDate) then
            ...
            let createAtomForIndexedItem (sortedRateChanges: InterestRateDefinition [], index, calculationEndDate) =
                ...
                let endDate =
                    if (index = sortedRateChanges.Length - 1) then
                        ...
                        sortedRateChanges[index + 1]
                            .RateStartDate.AddDays(-1)
            ...
            let atoms =
                Array.init atomsCount (fun index ->
                    createAtomForIndexedItem (sortedRateChanges, index, calculationEndDate))
    ...
    if (targetDate < interestCalculationAtoms[0].StartDate) then
        ...
        while (endDate <= maxDate) do
            if (targetDate >= startDate && targetDate <= endDate) then
                candidate <-
                    interestCalculationAtoms[iterator]
                        .EffectiveDailyRate
...
let createAtomForIndexedTransaction
    ...
    let dailyRateResult =
        activeRateForGivenDate (interestChangeAtoms, item.TransactionDate)
...
let calculateTransactionLoanAtoms (loan: Loan, calculationEndDate: DateOnly) =
    ...
    let interestChangeAtomsResult =
        calculateInterestChangeLoanAtoms (loan, calculationEndDate)
...
let interestChangeAtomsResult =
    calculateInterestChangeLoanAtoms (loan, loanCalculationEndDate)
